...
typedef struct _ARM_DRIVER_USART {
  ...
  int32_t          (*Stream)          (void *data);                        ///< Pointer to \ref ARM_USART_Stream : Start continuous receiving data from USART receiver.
...
/* Function definitions for mocks */

int32_t                driver_usart_Initialize(ARM_USART_SignalEvent_t cb_event);
int32_t                driver_usart_Uninitialize(void);
int32_t                driver_usart_PowerControl(ARM_POWER_STATE state);
int32_t                driver_usart_Send(const void *data, uint32_t num);
int32_t                driver_usart_Receive(void *data, uint32_t num);
int32_t                driver_usart_Stream(void* data);
int32_t                driver_usart_Transfer(const void *data_out, void *data_in, uint32_t num);
uint32_t               driver_usart_GetTxCount(void);
uint32_t               driver_usart_GetRxCount(void);
int32_t                driver_usart_Control(uint32_t control, uint32_t arg);
ARM_USART_STATUS       driver_usart_GetStatus(void);
